...
class TelegramNotifier:
    ...
    def _format_message(self, apt) -> str:
        """מעצב הודעת טקסט פשוטה (ללא Markdown) לדירה"""
        if apt.score >= 85:
            score_emoji = "🟢"
        elif apt.score >= 65:
            score_emoji = "🟡"
        ...
        lines = [
            f"🏠 דירה חדשה נמצאה! 🟠 יד2",
            f"",
            f"📍 {apt.neighborhood}, {apt.address}",
            f"🛏 {apt.rooms} חדרים | {apt.area_sqm} מר | קומה {apt.floor}",
            f"💰 ₪{apt.price:,} לחודש",
            f"",
        ]
        if apt.description:
            lines.append(f"📝 {apt.description[:200]}")
            lines.append("")
        lines.append(f"⏰ עלתה לפני: {time_ago}")
        lines.append(f"{score_emoji} ציון: {apt.score}/100")
        if apt.contact_name:
            lines.append(f"👤 {apt.contact_name}")
        if apt.url:
            lines.append(f"")
        ...
        try:
            ...
            if apt.image_url and self.config.get("send_photos", True):
                ...
                if r.status_code == 200:
                    ...
                    return True
                logger.warning(f"Photo failed ({r.status_code}): {r.text[:100]}")
            # שלח טקסט
            r = requests.post(
    ...
    def send_summary(self, apartments: list, total_scanned: int) -> bool:
        ...
        msg = (
            f"📊 סיכום סריקה\n"
            f"נסרקו: {total_scanned} | התאמות: {len(apartments)}\n"
            f"מחיר: ₪{min(a.price for a in apartments):,} - ₪{max(a.price for a in apartments):,}"
        )
        try:
    ...
    def send_startup_message(self) -> bool:
        try:
            r = requests.post(f"{self.api_url}/sendMessage",
                json={"chat_id": self.chat_id, "text": "🚀 דירה-האנטר הופעל! סורק דירות..."}, timeout=10)
            return r.status_code == 200
        except Exception:
            return False
